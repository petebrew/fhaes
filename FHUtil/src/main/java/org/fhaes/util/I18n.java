 ...
 * </pre>
 * 
 * Of course, you should internationalize this. But you want to change the text, the mnemonic, and the keyboard accelerator. If you only had
 * ResourceBundle to work with, you might end up with 3 lines of translation for every word in your program. Plus, you'd have to know the
 * name of the I18n file you used, each time you wanted to use it. Most other libraries (like Powerplant on Mac and Win95 resources) let you
 * put them all in one line, which makes the translator's job much easier. That's what this class does.
 * </p>
 * 
 * <p>
 ...
 * 
 * <li>"C" (the thing right after the &amp;) is the <i>mnemonic</i>. On most platforms (all except Mac), this letter is underlined, and
 * users can jump to it by pressing this letter, or Alt and this letter (depending on platform and context).
 * 
 * <li>"accel C" is the <i>keystroke</i>. The user can press this at any time to invoke this command. You can use the modifiers "shift",
 * "control", "alt", and "meta", or the special modifier "accel". "accel" is automatically turned into "control" on PCs (Windows and Linux),
 * and "meta" (Java's term for "command") on Macs. You should normally use the generic "accel" modifier. You can also combine modifiers,
 * like "shift accel S" (the standard keystroke for "Save As..."). The class javax.swing.KeyStroke takes these strings as input: you can say
 * 
 * <pre>
 ...
 * 
 * <p>
 * This is convenient, but it gets even better: normally, you don't even have to mess with keystrokes and mnemonics. You can simply use the
 * Builder factory to do the dirty work for you:
 * 
 * <pre>
 * 
 ...
 * 
 * <p>
 * These methods get their values from the resource bundle called "TextBundle". That is, the file is called "TextBundle.properties", or some
 * variant, like "TextBundle_de_DE.properties".
 * </p>
 * 
 * <h2>Left to do</h2>
 * <ul>
 ...
 * 
 * <li>Add way to escape [/]/&amp; in text; also, add markers so "this text is mnemonic/text/keystroke" and "this text is just a string" are
 * separate(?)
 * 
 * <li>in getKeyStroke(), why not use Toolkit.getDefaultToolkit().getMenuShortcutKeyMask() instead of (?:) for command/control choice?
 * </ul>
 * 
...
public class I18n {
	 ...
	 * 
	 * <p>
	 * For example, if the localization file has the line <code>copy = &amp;Copy [accel C]</code>, the string "control C" is returned (or on
	 * the Mac, "meta C").
	 * </p>
	 * 
	 * <p>
	...
	public static KeyStroke getKeyStroke(String key) {
		...
		String accel = (System.getProperty("os.name").startsWith("Mac") ? "meta" : "control");
	...
	/**
	 * Get the FHAESLocale using the country and language codes. If FHAES doesn't support the requested country/language, then the default
	 * FHAES locale is returned.
	 * 
	 * @param country
	 * @param language
	 * @return
	...
	/**
	 * Get the default locale for the system. If the systems locale is not supported 'fallback' determines if FHAES's default locale (US
	 * English) should be returned. If fallback is false, then null is returned if obtaining a locale is unsuccessful
	 * 
	 * @param fallback
	 * @return
	 */
	...
	public enum FHAESLocale {
		GERMAN("Deutsch", "de", "DE"), ENGLISH_PROPER("English (UK)", "en", "GB"), ENGLISH_US("English (US)", "en", "US"), FRENCH(
				"Français", "fr", "FR"), DUTCH("Nederlands", "nl", "NL"), POLISH("Polski", "pl", "PL"), TURKISH("Türk", "tr", "TR");
