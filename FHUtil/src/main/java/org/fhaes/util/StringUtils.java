...
/**
 * StringUtils Class. Some handy string utilities.
 * 
 * <h2>Left to do</h2>
 * <ul>
...
public class StringUtils {
	...
	/**
	 * Pad some text on the left (i.e., right-align it) until it's a specified width. If the text is already longer than the desired length,
	 * it is returned.
	 * 
	...
	/**
	 * Returns a user friendly string of the filename for a file shortened to the number of characters specified. Will try to keep the first
	 * folder and truncate in the middle if possible.
	 * 
	 * @param f
	...
	public static String shortedFileName(File f, Integer charactersallowed) {
		...
		try
		{
			...
			{
				String startpath = path.substring(0, path.indexOf(File.separator, 2) + 1);
				String endpath = path.substring(path.indexOf(File.separator, 2) + 1);

				Integer charstotrim = path.length() - (charactersallowed - name.length() - 4);
		...
		{
			return (path + File.separator + name).substring(0, charactersallowed);
		}
	...
	/**
	 * Given a string, escape any &lt; &gt; &amp; ' " characters for XML. Also, if any characters are unprintable, they're escaped as raw
	 * values (&amp;#xxxx;), so loading the output in any old text editor shouldn't mangle anything.
	 * 
