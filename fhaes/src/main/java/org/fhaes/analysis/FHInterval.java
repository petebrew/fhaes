...
public class FHInterval {
	...
	private void doAnalysis() {
		...
		if (run)
		{
			...
			double[] dfireintervalspersample;
			/*
			 * Set up either of the two filters two create the binary matrix on the case of composite analysis there are two possible
			 * filters: Number of fires and percentage of scarred trees.
			 */
			...
			for (int i = 0; i < myReader.size(); i++)
			{
				 ...
				 * array list subset of the years vector)
				 */
						...
						if ((FyearperSampletemp.size() >= 2))
						{
							...
							if (includeIncomplete)
							{
										...
										.get(FyearperSampletemp.size() - 1))
								{
									...
									FIyearperSampletemp
											.add((myReader.get(i).getFirstYear() + myReader.get(i).getLastYearIndexPerSample()[k])
													- FyearperSampletemp.get(FyearperSampletemp.size() - 1));
