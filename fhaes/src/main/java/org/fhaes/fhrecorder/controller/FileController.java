 ...
 *     2) Changes were made in doSaveFileFunctionality() in order to address
 *     general error #3 on our original list of maintenance requests.
 *     3) If you attempt to get the earliest event year for a sample with
 *     no events, the method now returns EARLIEST_ALLOWED_YEAR instead of
 *     crashing.
 *     4) Method added to check if the current sample has any events.
 ******************************************************************************/
			...
			if (!filePath.toLowerCase().endsWith(".fhx"))
			{
				...
				if (selectedFile.getName().length() > FHX2_MAX_FILE_NAME_LENGTH && isEnforcingOldReqs() == true)
				{
					FireHistoryRecorder.updateStatusBarMessage(MessageType.WARNING, Color.red,
							StatusBarPanel.NO_SPECIFIED_MESSAGE_ID, "File name is too long for the original FHX program requirements.");
